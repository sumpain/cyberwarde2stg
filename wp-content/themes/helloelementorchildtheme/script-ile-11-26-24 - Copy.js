(function($){
					...
					if($(this).text().toLowerCase() == 'graduate'){
						...
						if(!$(this).hasClass('clicked')){
							...
							if(sp_obj.course.id !== false && sp_obj.course.form !== false && sp_obj.lesson.id !== false){							
								/*Added by RM Jan 22 2025*/
									var g_userId = sp_obj.user.id;
									var g_lessonId = sp_obj.lesson.id;								
									var g_courseId = sp_obj.course.id;								
									jQuery.ajax({
										url: sp_obj.ajax_url,
										method: 'POST',
										data: {
											action: 'update_user_course_status',
											user_id: g_userId,
											lesson_id: g_lessonId,
											course_id: g_courseId
										},
	...
	$(document).ready(function() {
	    ...
	    $(document).on("click",".otpestatus .generateOtp",function() {
	    	console.log('class generateOtp is click');
